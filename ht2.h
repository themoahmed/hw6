...
public:
   ...
   HASH_INDEX_T next()
   {
     ...
     //iterate by the second hashfunction step size
...
private:
   ...
   //resizeAlpha and entry numbers
   double rA;
   HASH_INDEX_T activeEntries;
   size_t allEntries;
};
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{  
   //delete every value that isn't a nullptr
...
{ 
   //calculate alpha USING ALL ENTRIES, INCLUDING DELETED
   double alpha = static_cast<double>(allEntries)/table_.size();
  
   //if alpha becomes larger call resize
   if (alpha >= rA){
       resize();
   }
   //find the location of the itemp
   HASH_INDEX_T idx = this -> probe(p.first);
   ...
   //update the second part of the item
   else if (table_[idx] != nullptr) {
       if(table_[idx] -> item.first == p.first){
           table_[idx] -> item.second = p.second;
       }
   }
   //otherwise create new item and increment the active and all entries by 1
   else if(table_[idx] == nullptr){
...
{
   //find the location
   HASH_INDEX_T idx = this->probe(key);
   //just make the deleted bool true and then lower active entries by 1
...
{
   mIndex_++;
   //if reach end of CAPACITIES array
   if(mIndex_ == 28){
     return;
   }
   //get the amount of active entries used
   size_t numEntries = 0;
   //new size is next in the capacities array
   HASH_INDEX_T newSize = CAPACITIES[mIndex_];
   //hold the current table vector
   std::vector<HashItem*> tempHash = table_;
   table_.clear();
   //resize to new size
   table_.resize(newSize);


   //iterate through the temp vector
   for (size_t i = 0; i < tempHash.size(); i++){
     //if the index has an activeEntry
     if(tempHash[i] != nullptr and !tempHash[i]->deleted){
       //find the item
       HASH_INDEX_T idx = this->probe(tempHash[i] -> item.first);
       if(idx != npos){
         //add to the table
         table_[idx] = tempHash[i];
         //increase entries by 1
         numEntries++;
       }
     }
     else{
       //otherwise delete because inactive
       delete tempHash[i];
     }
     //set everything to numEntries
     activeEntries = allEntries = numEntries; 
   }
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
   ...
   while(Prober::npos != loc)
   {
       ...
       else if(table_[loc] -> item.first == key and table_[loc] -> deleted == false) {
           return loc;
       }
       loc = prober_.next();
       totalProbes_++;
